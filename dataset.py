...
class ArticulationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.labels_data is None:
            self.labels_data = h5py.File(
                os.path.join(self.root_dir, 'complete_data.hdf5'), 'r')
        ...
        obj_pose_in_world = np.array(
            obj_data['embedding_and_params'])[-7:]  # obj_pose, obj_quat_wxyz
        ...
        for i in range(len(moving_body_poses) - 1):
            ...
            new_l = transform_plucker_line(np.concatenate((l_hat, m)),
                                           trans=obj_T_pt1[:3],
                                           quat=obj_T_pt1[3:])
            label[i, :] = np.concatenate(
                (new_l, [theta], [d]))  # This defines frames wrt pt 1

        # Normalize labels
        label[:, 3:6] /= self.normalization_factor

        label = torch.from_numpy(label).float()
        sample = {'depth': depth_imgs, 'label': label}
...
class RigidTransformDataset(Dataset):
    ...
    def __getitem__(self, idx, imgs_per_object=16):
        ...
        depth_imgs = torch.tensor(
            [obj_data['depth_imgs'][0], obj_data['depth_imgs'][obj_data_idx]])
        depth_imgs.unsqueeze_(1).float()
        depth_imgs = torch.cat((depth_imgs, depth_imgs, depth_imgs), dim=1)
