...
def articulation_lstm_loss_spatial_distance(pred, target, wt_on_ortho=1.):
    ...
    pred = pred.view(
        pred.size(0), -1,
        8)[:, 1:, :]  # We don't need the first row as it is for single image
    ...
    # Configuration Loss
    conf_err = theta_config_error(target, pred)**2 + d_config_error(
        target, pred)**2
    ...
    loss += wt_on_ortho * torch.mean(
        torch.abs(torch.sum(torch.mul(pred[:, :, :3], pred[:, :, 3:6]),
                            dim=-1)))
    ...
    if torch.isnan(loss):
        ...
        print("Distance loss:{}".format(
            torch.mean(
                orientation_difference_bw_plucker_lines(target, pred)**2)))
        print("Orientation loss:{}".format(
            torch.mean(distance_bw_plucker_lines(target, pred)**2)))
...
def articulation_lstm_loss_L2(pred,
                              target,
                              wt_on_ax_std=1.0,
                              wt_on_ortho=1.,
                              extra_indiv_wts=None):
    """ L2 loss"""
    pred = pred.view(
        pred.size(0), -1,
        8)[:, 1:, :]  # We don't need the first row as it is for single image
